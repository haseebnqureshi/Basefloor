...
Why? Because these uploads have certain labels that are
applied for the relevance of whatever parent item is 
requiring. 

For example, uploading an invoice can have a certain 
name as the object key, with then the file _id (ObjectID)
allowing the app to pull more information on the file
resource:

{ 'Invoice #23423' : ObjectID(_id of a file item) }
*/
...
const isEnabled = () => field.type == 'uploadobject' ? true : false
...
let newRow = (i) => ({ idle: true, message: '', label: `` })
...
if (isEnabled()) {
  ...
  if (value) {
    // we have an object with labels as keys and _ids as values
...
const rowsToValue = () => {
  ...
  for (let i in rows.value) {
    ...
    let label = row.label != '' ? row.label : `#${(Number(i)+1).toString()}`
    val.value[label] = row._id || null //@todo: if file is removed, have null -- but the API doesn't accept null, and so we lose the label. Want to keep the label for saving drafts that are precisely accurate.
  }
}
    ...
    <div v-for="(row, index) in rows" class="mt-2 relative block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">
      ...
      <div class="ml-8 mr-14 my-2 relative">
        ...
        <input 
          ...
          placeholder="Please provide label or name"
        />
